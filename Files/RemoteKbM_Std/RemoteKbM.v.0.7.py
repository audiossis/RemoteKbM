...
class Settings:
	def run(self):
		...
		for i in mod_key_table:
			#print str(i)
			sender.put_nowait(str(i) + "_ANTRls" + "0\n")
		
		for i in key_fonts:
			#print "Fonts..."
			key_font = key_fonts[i]
			sender.put_nowait(str(i) + "_FontName_" + str(key_font) + "\n")
...
class Processing(threading.Thread):
	...
	def run(self):
		...
		key_measure = str(datetime.datetime.now() - last_key_time).split(":", 1)[1].split(":", 1)[1]
		...
		while running:
			...
			if (CY is not None and CX is not None) and float(measure)<0.125:
				...
				if float(measure)<0.025 and float(measure)>0.0175:accelX,accelY=6,5
				...
				if (current_pos[0]>0 and current_pos[0]<screen_dim[0]) and (current_pos[1]>0 and current_pos[1]<screen_dim[1]):
					if (targetX<screen_dim[0]) and (targetX>0) and (targetY<screen_dim[1]) and (targetY>0):
						...
						elif CX < 123 and CY < 62:m.move((targetX),targetY)
						elif CX >= 124:m.scroll(int(dirY),0,0)
						elif CY >= 63:m.scroll(0,int(dirX),0)
				elif (current_pos[0]<=0) or (current_pos[1]<=0):
					if dirX>0 and CX < 123 and CY < 62:
						m.move((1+dirX),current_pos[1])
					elif dirY>0 and CX < 123 and CY < 62:
						m.move(current_pos[0],(dirY+1))
					elif dirY>0 and CX >= 124:m.scroll(int(dirY),0,0)
					elif dirY>0 and CY >= 63:m.scroll(0,int(dirX),0)
			...
			key_measure = str(datetime.datetime.now() - last_key_time).split(":", 1)[1].split(":", 1)[1]
			if float(key_measure) > float(30):
				#print str(key_measure)
				Settings().run()
				last_key_time = datetime.datetime.now()
